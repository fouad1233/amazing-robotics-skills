<!-- Vendored from Seeed-Projects/Seeed-Jetson-DevelopTool @ main
     Path:    skills/openclaw/openclaw-local-deploy/SKILL.md
     Licence: MIT
     Source:  https://github.com/Seeed-Projects/Seeed-Jetson-DevelopTool/blob/main/skills/openclaw/openclaw-local-deploy/SKILL.md
     Unmodified copy. See NOTICE for attribution. -->

---
name: openclaw-local-deploy
description: Deploy OpenClaw (Clawdbot) locally on reComputer Jetson with Ollama for a self-contained AI control hub. Covers Ollama installation, model pulling, OpenClaw setup, and local model configuration. Requires Jetson with ≥16GB RAM and JetPack 6.2.
---
...
| Config file syntax error | Validate JSON: `python3 -m json.tool ~/.openclaw/openclaw.json`. Fix trailing commas. |
| WebUI not loading | Check gateway is running: `openclaw gateway status`. Verify port 18789 is not blocked. |
| "unable to access" error on WebUI | Token authentication is enabled. Find token in `~/.openclaw/openclaw.json` under `gateway.auth.token` and append to URL. |
